...
#torchrun --standalone --nproc_per_node=1 train_gpt2v2.py
...
#LOG_DIR=/Data/theophile.laurent/datasets/gpt2_runs/$(date +%Y%m%d_%H%M%S)/log \
#torchrun --standalone --nproc_per_node=1 train_gpt2v2.py
...
min_lr =  max_lr * 0.1 
warmup_steps = 715
max_steps = 19073  #original : 19073       # 19,073 steps is ~1 epoch, if data is 10B tokens and batch size 0.5M tokens
def get_lr(it) :
...
if master_process:
    ...
    # si val_loss_accum n'existe pas (cas où aucune val ne s'est encore exécutée), on met None
